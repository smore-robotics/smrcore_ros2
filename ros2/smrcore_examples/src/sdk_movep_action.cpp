...
class SdkMovePActionExample : public rclcpp::Node
{
public:
    using ActionT = smrcore_msgs::action::MoveP;
    using GoalHandle = rclcpp_action::ClientGoalHandle<ActionT>;

    SdkMovePActionExample() : Node("smrcore_sdk_movep_action_example")
    {
        action_name_ = declare_parameter<std::string>("action_name", "/smrcore_sdk_server/move_p");
    ...
    void Run()
    {
        ...
        {
            RCLCPP_ERROR(get_logger(), "等待 MoveP action server 超时: %s", action_name_.c_str());
            rclcpp::shutdown();
            return;
        }
        ...
        options.result_callback = [this](const GoalHandle::WrappedResult &result) {
            if (result.result)
            {
                RCLCPP_INFO(get_logger(), "MoveP result: success=%s error_code=%u message=%s",
        ...
        RCLCPP_INFO(get_logger(), "发送 SDK MoveP action 目标");
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<SdkMovePActionExample>();
    node->Run();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
